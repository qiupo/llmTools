...
if ! command -v uv >/dev/null 2>&1; then
    echo "error: uv is required to install the isolated Fun-ASR-Nano mlx-audio runtime." >&2
    echo "Install uv first or set LLMTOOLS_FUN_ASR_NANO_VENV to an existing venv with mlx_audio.stt.generate and mlx_audio.stt.models.fun_asr_nano." >&2
    exit 69
fi
...
if [ -z "$PYTHON_BIN" ]; then
    if command -v python3.12 >/dev/null 2>&1; then
        ...
        echo "error: python3.12 or python3.11 is required for the Fun-ASR-Nano mlx-audio runtime." >&2
        exit 69
    fi
fi
...
echo "Installed llmTools Phase 4 Fun-ASR-Nano mlx-audio runtime:"
echo "  venv: $VENV"
echo "  runner: $RUNNER_PATH"
echo
echo "Example command template:"
echo "  LLMTOOLS_FUN_ASR_NANO_VENV='$VENV' '$RUNNER_PATH' --model {model} --audio {audio} --language {language}"
