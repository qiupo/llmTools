...
enum L10n {
    ...
    private static let chinese: [String: String] = [
        ...
        "No usable model configured": "未配置可用模型",
        "No usable text model is configured. Open Models > Get Started to download or add one.": "未配置可用文本模型。请打开“模型 > 开始使用”下载或添加模型。",
        "Model registry could not be loaded": "模型注册表无法读取",
        "Model setup needs attention": "模型配置需要处理",
        "Model setup required": "需要配置模型",
        "Get Started": "开始使用",
        "Open Model Setup": "打开模型配置",
        "Not installed": "未安装",
        "Model files missing": "模型文件缺失",
        "Preparing subtitles": "正在整理字幕",
        "Subtitle translation needs a text model or a ready Fast MT runtime.": "字幕翻译需要文本模型或已就绪的 Fast MT runtime。",
        "Text tasks need a usable text model. Open model setup to download or add one.": "文本任务需要可用的文本模型。请打开模型配置下载或添加。",
        "Image recognition needs a usable vision model. Open model setup to download or add one.": "图片识别需要可用的视觉模型。请打开模型配置下载或添加。",
        "Media transcription needs a usable local ASR model. Open model setup to download or add one.": "媒体转写需要可用的本地 ASR 模型。请打开模型配置下载或添加。",
        "Speech generation needs the TTS runtime and a VoxCPM2 model. Open model setup to install them.": "语音生成需要 TTS runtime 和 VoxCPM2 模型。请打开模型配置安装。",
        "Start with one recommended model for each feature you need. You do not need to download every supported model.": "按需要使用的功能各安装一个推荐模型即可，不需要下载全部支持模型。",
        ...
        "Assistant Display": "助手显示",
        "Enable Assistant": "启用助手",
        "Disable Assistant": "停用助手",
        "Show Assistant": "显示助手",
        "Hide Assistant": "隐藏助手",
        "Show on all regular Spaces": "在所有普通桌面显示",
        "Show over full-screen apps": "在全屏应用上显示",
        "Toolbar trigger": "工具条触发方式",
        "Hover": "悬停",
        "Click": "点击",
        "Current Session": "当前会话",
        "Recent Suggestions": "最近提示",
        "Clear Suggestions": "清除提示",
        "Recent suggestions are kept only for this app session, up to 20 cards.": "最近提示只保留在当前应用会话中，最多 20 张卡片。",
        "Paused": "已暂停",
        "Privacy Mode": "隐私模式",
        ...
        "Enable Desktop Context Assistant": "启用桌面情境助手",
        "Show Desktop Context Assistant": "显示桌面情境助手",
        "Quit": "退出"
    ]
}
