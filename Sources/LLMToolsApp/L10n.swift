...
enum L10n {
    ...
    private static let chinese: [String: String] = [
        ...
        "Used by text-mode LLM actions. Webpage translation can choose its own model on the Webpage tab.": "用于文本模式的 LLM 动作。网页翻译可以在网页页签单独选择模型。",
        "History": "历史",
        "Application": "应用",
        "Version": "版本",
        "Data": "数据",
        ...
        "Use {audio_wav_16k_mono}, {output_json}, and {hf_token}. Empty field uses the bundled pyannote sidecar when the local runtime is installed.": "可使用 {audio_wav_16k_mono}、{output_json} 和 {hf_token}。留空时，在本地运行时已安装的情况下使用内置 pyannote sidecar。",
        "Use {audio_wav_16k_mono}, {output_json}, {diarization_model}, {hf_cache}, and {hf_token}. Empty field uses the bundled pyannote sidecar when the local runtime is installed.": "可使用 {audio_wav_16k_mono}、{output_json}、{diarization_model}、{hf_cache} 和 {hf_token}。留空时，在本地运行时已安装的情况下使用内置 pyannote sidecar。",
        ...
        "Only Translate can use fast MT. Polish, summary, explanation, and TODO always use the default LLM model.": "只有“翻译”可以使用快速 MT。润色、总结、解释和待办始终使用默认 LLM 模型。",
