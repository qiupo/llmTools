...
状态：阶段 A、B、C、`D-73`（P-06）及 `D-74` 至 `D-83`（前台窗口视觉陪伴、无动作对白气泡、实时诊断、可操作徽标、工作动效、可调门槛和人在场启停）均已实现；新增源码、packaged app、签名和真实 ScreenCaptureKit 取图时机均已验证。prompt `18` 已由真实本地 `Qwen3.5-9B-MLX-4bit` 通过完整资格门槛，当前为“活跃”且有 `1` 个 qualified 模型。
...
flowchart LR
    ...
    Privacy --> Detector["模式检测"]
    Detector --> Judgment["较强本地模型价值复核"]
    Judgment --> Policy["打扰策略"]
    Policy --> Comment["模板或本地评论模型"]
    Comment --> Cards["提示卡"]
    Cards --> Orb["悬浮球 / 瞥见卡"]
...
- P-01、P-03 的固定阈值边界和正反例，以及 `D-73` 追加的 P-06 授权来源、`8s` 合并、同情境 `10min` 冷却和证据锁定边界；不存在 P-02 检测器、设置或历史迁移逻辑。
...
- 资格检查保持 `100%` JSON 有效，隐私、敏感、全屏和证据不足硬反例不允许主动误报；其余负例最多误报 `1` 个，正例至少通过 `9/12`。
...
- 评论输出只允许改变表达和选择已有动作；`3s` 超时、资源冲突、非法输出和敏感表达均使用模板降级。
...
- Assistant 页面承载行为、上下文和隐私；判断/评论模型实际配置放入 Models > Model Settings。
...
- 阶段 C 已完成：V1 包含 P-01、P-03，以及用户按 `D-73` 追加批准的 P-06；主动程度、独立冷却、反馈、评论契约、较强本地模型价值判断、`24` 个共享资格 fixture、资格缓存和保守资源降级已经实现。fixture 保持总数 `24`，由 P-01/P-03/P-06 各 `4` 个正例和 `12` 个反例组成，P-06 另有 `3/4` 的最低门槛。
- 视觉陪伴增量已完成：系统最后输入时间前进后 `3s` 防抖触发前台单窗口截图，最短间隔 `15s`；截图、视觉理解、固定 `8s` 聚合、价值判断、性格表达和展示按单轮串行执行。聚合窗口由首条有效证据固定，异步任务晚醒不会排除窗口首条证据；同轮新证据直接并入且不重置计时，运行期间的新操作或新上下文最多合并为一次下一轮请求，当前轮结束后再启动；连续空闲 `60s`、锁屏、会话失活或睡眠时暂停。`1600px` 缩放、内容哈希、内存 PNG、本地通用 VLM、严格场景 JSON、短期上下文和无动作 P-06 评论均已接入。
- 无动作 `contextualOpportunity` 复用既有非激活面板呈现 `280pt` 纯文字对白气泡；按悬浮球相对位置切换尾巴方向，点击仅收起，带动作提示继续使用完整瞥见卡。
- 只有未读且带动作入口的卡片计入徽标；截图、视觉理解、价值判断、性格表达和显式快问真实运行时，悬浮球显示尊重“减少动态效果”的彩色工作环。
- 高级诊断展开时每秒刷新 schema `3` 状态；最新 `24` 条内存事件覆盖人在场、截图、视觉理解、情境聚合、价值判断、性格表达和展示，并显示默认或自定义置信度门槛。连续操作会原位刷新待执行截图的计划时间；界面同时展示可读解释和原始机器码，JSON 导出同时保留 `recentActivity` 与可读 `recentActivityDescriptions`，并分别标识无证据、相同情境冷却及策略静默；收起即停止轮询，事件及 JSON 不含应用标识或观察正文。
...
- `D-74` 至 `D-83` 新增检查已通过：场景 JSON 接受受限事实并拒绝敏感摘要；P-06 可在证据充分时无动作展开；评论按性格生成且无按钮，URL、代码和敏感表达仍被拒绝；只有空动作 P-06 进入对白气泡分支且不计徽标；自定义置信度受 `0.50...0.95` 约束；诊断时间线只保留最新 `24` 条并维持倒序，同一待执行计划原位刷新；聚合任务晚醒 `0.2s` 时仍保留固定窗口边界上的首条证据。
