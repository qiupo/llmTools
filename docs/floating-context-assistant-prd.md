...
状态：开发前设计评审已完成。`D-01` 至 `D-83` 已确认；`D-73` 至 `D-83` 是用户在交付验收期间批准的增量决策，并只覆盖其中明确列出的旧冲突。
...
- 等待期间原始上下文过期后，不再运行判断。
- 判断总时限 `8s`，模型就绪后的生成最多 `5s`；超时、取消、内存不足、输出非法或进入暂停时降级为徽标。
- 当前已加载模型只有通过判断能力检查时才能复用；后台判断永久禁止远程 provider。
- 判断完成后沿用全局闲置卸载策略，不为助手永久常驻模型。

`已确认 D-57`：按主动程度决定是否调用价值模型；后台判断单并发、队列最多 `3`、总时限 `8s`/生成 `5s`，永远让位于用户主动任务。资源冲突、过期和失败统一降级，不能调用远程 provider 或复用未合格模型。
...
建议长度：中文 `12-60` 字；超过后应压缩或进入详细视图。

评论生成顺序：

1. 价值判断先锁定事实、展示级别和允许动作，评论生成不能修改这些结果。
2. 默认优先复用当前已加载且可用的本地模型，不为每张卡强制切换到第二个模型。
3. 用户覆盖了助手评论模型时，只在资源空闲且不影响用户主动任务时调用。
4. 评论生成最多等待 `3s`；超时、资源冲突、模型不可用或输出非法时直接使用确定性模板。
5. 专业和温和风格必须有完整模板；轻度吐槽允许模型润色，但模板仍须满足最多一个玩笑点。

`已确认 D-61，细化 D-23`：评论引擎保持独立职责和可覆盖设置，但不要求每张卡加载独立模型。默认复用已加载的可用本地模型，`3s` 内不能得到合格结果就使用模板；评论不能改变判断事实、展示级别和允许动作。
...
| 助手判断模型 | 对预筛选候选做高价值、证据和打断成本复核 | 是，要求较强本地模型 |
| 助手评论模型 | 在不改变事实的前提下生成专业、温和或吐槽表达 | 是，可使用较轻本地模型 |
...
- 允许单独选择“助手评论模型”，但默认优先复用当前已加载的可用本地模型。
- 用户覆盖的评论模型只在资源空闲时调用，不为评论抢占用户主动任务。
- 没有可用本地模型时使用固定模板，不阻断悬浮球。
- 不因为悬浮球常驻而永久加载文本模型。
- 模式检测先完成，只有候选提示达到阈值时才加载或调用模型。
- 评论完成后沿用 app 全局模型闲置卸载策略。
- 长任务交给已有工作台默认模型，不强制使用评论模型。
- 高价值判断使用独立职责和更高能力门槛，默认不与轻量评论模型绑定。
- 评论最多等待 `3s`，不能及时生成时使用模板；不能为了节省资源降低价值判断质量。
...
- 判断完成后沿用全局闲置卸载策略。
- 不要求判断模型和评论模型同时常驻。
- 判断模型可以返回事实化建议，再由确定性语气模板完成表达。
- 加载失败、超时或资源不足时立即保守降级，不阻塞悬浮球交互。

`已确认 D-23，按 D-61 细化`：助手评论采用独立职责和可覆盖设置，不直接绑定普通文本默认模型，也不强制加载第二个模型。评论生成与 `D-08` 的高价值判断职责分开，不能因为评论生成快就让它决定是否主动打扰。
...
```json
{
  "comment": "这条错误已经第三次出现了。要不要先检查端口占用？",
...
- 评论不能修改判断模型已经锁定的证据、展示级别和动作集合，也不能引入输入中不存在的数字、应用和人员。
- 校验失败时回退到固定模板。
- 评论模型不需要取得 `D-59` 的主动判断资格，但每次输出仍必须通过结构、事实、敏感表达和长度校验。
...
折叠区显示助手启停/暂停/隐私模式、各观察器状态和最后事件时间、内存队列数量、判断队列及让路状态、判断模型资格与检查版本、行为存储健康状态，以及当前临时翻译会话剩余时间。展开时每秒刷新，并额外显示截图计划/尝试/成功、视觉理解、8 秒情境聚合、价值判断、性格表达和最终展示的当前状态及最近有界事件。连续操作只刷新同一条待执行截图的真实起算时间；聚合计划显示从首条证据起算的窗口剩余时间；界面在原始机器码之外补充可读解释，并明确区分无证据、相同情境冷却和后续策略静默。
...
- 本地评论模型冷启动允许显示“正在整理建议”，但不能阻塞悬浮球交互。
- 评论目标为中位 `2s` 内返回，最迟 `3s` 后使用固定模板。
- 不为了等待评论模型而延迟明确动作，例如打开 OCR 或媒体转写。
- 评论模型沿用全局闲置卸载策略，不默认永久驻留。
- 事件聚合使用有界队列；超过容量时丢弃低价值旧事件，不能无限增长。
- app 退出、暂停助手或关闭对应来源时停止观察并释放相关资源。

`已确认 D-28`：只有检测到高概率候选模式后才允许短时预热评论或判断模型，不在 app 启动时加载。
...
| 没有本地评论模型 | 使用规则模板 |
| 评论模型超时或输出非法 | 使用规则模板，不显示原始模型错误给普通用户 |
...
| 编号 | 问题 | 最终结论 | 状态 |
| --- | --- | --- | --- |
| D-73 | 是否加入 P-06，以及首次启用默认使用什么主动程度 | 加入 P-06 情境机会；普通首次启用默认适中并在引导明确说明后自动运行 24 项本地资格检查，仅手动预设仍为手动。无合格模型、检查中、失败、超时、非法输出或证据不可核实时实际保持安静。P-06 只合并用户已授权的剪贴板、显式选区和脱敏窗口标题变化，前台应用元数据只能辅助归类且不能单独触发；8 秒合并、同情境 10 分钟冷却，只允许合格本地判断模型锁定原证据片段并选择现有 Quick Action 文本任务。评论模型只能选择事实锁定的表达模板，不能改变事实、依据或动作。该决策只覆盖 `D-17`、`D-55`、`D-66`、`D-71` 中与默认安静、V1 仅 P-01/P-03 和首次资格检查触发冲突的部分，其余冻结结论继续有效。 | 已确认 |
...
| D-79 | 如何实时调试助手是否截图、思考及为何没有展示 | “高级诊断”展开时每秒刷新现有诊断快照，显示截图调度与尝试、视觉理解、情境聚合、价值判断、性格表达和展示状态，并在内存保留最新 `24` 条阶段/状态/机器原因码；待执行截图被新操作防抖延期时原位刷新计划时间，界面为机器码补充可读原因，并区分无证据、相同情境冷却和策略静默。诊断 JSON 同时导出原始 `recentActivity` 和同一批事件的可读 `recentActivityDescriptions`。收起后停止轮询。事件和导出不得包含应用标识、截图、窗口标题、识别正文、内容指纹或模型标识。 | 已确认并实现 |
| D-80 | 哪些提示计入悬浮球未读徽标 | 只有仍未读且带动作入口、需要用户处理的卡片计数并显示列表圆点；无动作陪伴表达仍保留在最近提示，但不形成数字徽标，也不在点击悬浮球时作为待办重新打开。 | 已确认并实现 |
| D-81 | 如何表示助手正在工作 | 截图、视觉理解、价值判断、性格表达或显式快问真实运行时，`48pt` 悬浮球圆环显示彩色旋转并让图标轻微脉冲；排队和 8 秒聚合不触发动效，系统“减少动态效果”开启时改为静态彩环。 | 已确认并实现 |
...
| D-83 | 截图和主动助手何时因人在场状态启停 | 取消 `20s` 固定截图心跳。复用系统“距最后输入时间”元数据，只有时间戳前进才在 `3s` 防抖后安排截图，并保持 `15s` 最短截图间隔。截图、视觉理解、固定 `8s` 聚合、价值判断、性格表达和展示视为一轮串行操作：聚合窗口固定为首条有效证据发生时间至其后 `8s`，任务晚醒不得改变窗口边界；同轮新证据不重置计时。“没有匹配功能”由后续价值判断选择无按钮表达或静默，不得在聚合阶段淘汰；只有没有可信语义证据或相同情境仍在冷却期时才在聚合阶段结束。本轮未结束时，新操作和新上下文各自最多合并为一次下一轮请求，当前轮结束后才启动。连续空闲 `60s`、锁屏、会话失活或睡眠时取消当前及待处理轮次；不得读取或保存按键、坐标和鼠标轨迹。 | 已确认并实现，修订 D-74 |
