...
ASR 仅本地运行。Phase 4 没有远程 ASR 设置，也没有云端 ASR fallback。配置本地 Fun-ASR 流式运行时后，Fun-ASR-MLT-Nano 是多语言实时字幕的优先家族；Fun-ASR-Nano 可用于更低延迟的中文/英文/日文实时字幕。SenseVoiceSmall 仍保留为短窗低延迟 ASR；Qwen3-ASR-0.6B 可用于文件转写，也可以在本地 vLLM/流式运行时就绪后作为实验性实时 ASR 选择。Qwen3 实时模式默认采用更保守的最终字幕策略。
