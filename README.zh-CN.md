...
最新版本：[v0.4.0](https://github.com/qiupo/llmTools/releases/tag/v0.4.0)
...
llmTools 仍在持续开发中。当前 `v0.4.0` 版本已经包含桌面 Quick Action、本地/远程模型注册表、模型能力设置、本地模型视觉 OCR、Chromium 网页翻译、媒体字幕、原生桌面实时字幕、本地语言/fast-MT 路由、文件说话人分离，以及全本地会议转写与纪要。

会议转写与低延迟实时字幕是两条独立管线。实时会议支持麦克风或原生系统音频，本地音视频文件走离线处理；`v0.4.0` 不包含麦克风与系统音频混合的会议采集。实时字幕的说话人分离仍然硬禁用：当前 MVP 不持久化 speaker embedding，也不做跨文件说话人身份识别。
...
官方 Fun-ASR 加速路径按硬件分开：vLLM 路径面向 CUDA/NVIDIA 服务端，吞吐和流式服务能力最强；llama.cpp/GGUF 路径面向 CPU/edge/本机运行，依赖 `llama-funasr-cli` 单二进制和可选内置 FSMN-VAD。当前核对到的 Fun-ASR GitHub 文档没有把 Apple MPS/Metal 作为明确支持的加速路径，因此 llmTools 不会默认假设 Fun-ASR 可以用 `device="mps"` 加速。
...
命令需要输出纯文本转写，或形如 `{"segments":[{"start":0,"end":2.5,"speakerID":"0","speakerLabel":"Speaker 1","text":"Hello"}]}` 的 JSON 字幕片段。`{audio}` 是归一化后的 16 kHz mono WAV 路径，`{model}` 是选中的本地模型目录。Settings 命令优先于环境变量。Fun-ASR GGUF 目录在 `PATH` 中存在 `llama-funasr-cli`，且模型目录包含兼容的 Fun-ASR encoder 与 Qwen3 decoder GGUF 文件时可以自动使用。SenseVoiceSmall 也可以在模型目录包含 `model.onnx` 和 `tokens.txt` 且 `PATH` 中存在 `sherpa-onnx-offline` 时自动使用 sherpa-onnx。safetensors/MLX ASR 目录通过 app 内置的 `llmtools-mlx-asr-runner.sh` 调用按模型族隔离的 runtime：Qwen3 和 mlx-community VibeVoice-ASR 使用固定版本 `mlx-audio`，SenseVoiceSmall 和 Fun-ASR-Nano 使用补丁后的 `mlx-audio`，Fun-ASR-MLT-Nano 使用 `mlx-audio-plus`。mlx-community VibeVoice-ASR 还需要本地 Qwen2.5 tokenizer sidecar；`install-phase4-mlx-asr-runtime.sh` 会安装到 `~/Library/Application Support/llmTools/asr-runtime/qwen2.5-tokenizer`，也可以用 `LLMTOOLS_VIBEVOICE_TOKENIZER_DIR` 覆盖。原始 PyTorch VibeVoice-ASR runtime 仍可通过 `scripts/llmtools-vibevoice-asr-runner.py` 或自定义命令接入，并保留模型 rich transcription 的说话人/时间戳字段。
...
健康检查会显示 runtime 来源：Settings 命令、环境变量、测试夹具、本地 MLX runner、自动 sherpa-onnx，或未配置。对于受支持的 safetensors/MLX 模型目录，Settings 健康检查会在匹配 runtime 缺失时提供修复按钮。
...
`v0.4.0` 暂不支持会议中的麦克风与系统音频混合采集。原生说话人模型把自然停顿保留为逻辑讲话边界，但连续讲话每 120 秒会封装一个有界的技术推理窗口；普通 ASR 优先在自然停顿处输出，并限制连续讲话的最大等待时间。如果本地 ASR 慢于采集且已经排队 2 个推理窗口，app 会自动停止采集并完成队列，避免内存继续无界增长。正常停止时默认删除临时会议音频；异常退出后的清理只处理已终止进程拥有的音频，不会误删另一个仍在运行的 app 实例。
...
| 安装 legacy/original PyTorch VibeVoice-ASR runtime | `./scripts/install-phase4-vibevoice-asr-runtime.sh` |
...
也可以在 GitHub Actions 手动运行 workflow，并传入类似 `v0.4.0` 的 `version` input。
