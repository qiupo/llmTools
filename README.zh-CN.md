...
- 原生 macOS SwiftUI/AppKit 菜单栏应用，Quick Action 支持文本、图片、媒体、语音四种模式及可配置快捷键。
- 选中文本工作流：翻译、写作润色、总结、解释、TODO 提取；Quick Action 翻译还提供候选译法、重点词汇、发音、词性、用法、例句和本地朗读。
...
- 全本地 VoxCPM2 bf16/4bit 文案转语音：Quick Action 可直接输入文字、选择音色与语气、试听、播放并导出 WAV/M4A；独立工作台继续支持可审阅的多角色脚本、断点续生成和 SRT 导出。
...
| 功能 | 入口 | 使用方法 |
| --- | --- | --- |
| 选中文本 Quick Action | 在其他应用选中文字后按 `Option + Space` | 选择翻译、润色、总结、解释或提取 TODO；详细翻译优先使用已启用的本地质量模型，结果包含候选译法、重点词汇和语言提示。原文、译文与重点词统一使用已固化的“温柔女旁白”朗读；生成中点击其他朗读项会取消当前请求并切换。需要自动读取选区时授予 Accessibility 权限。 |
| 粘贴文本与文件 | 状态栏菜单 -> `打开快捷操作` 或 `打开悬浮组件` | 粘贴文本/图片或拖入支持的文件，选择任务和模型，然后复制或导出结果。 |
| 快速生成语音 | Quick Action -> `语音` | 输入文字，选择音色和语气，可先试听所选音色，再生成本地语音并播放或导出 WAV/M4A。 |
...
| 本地文案转语音 | 状态栏菜单 -> `文案转语音` | 输入普通或长文案，按需切换多角色并审阅脚本；在独立“音色管理”窗口设计/克隆、固化和试听音色，然后生成、暂停/继续试听并导出 WAV/M4A/SRT。 |
...
| 本地 VoxCPM2 TTS 安装 | `uv`、Python 3.11 或 3.12、`mlx-audio==0.4.5`，以及 VoxCPM2 bf16 或 4bit 权重 |
...
| `Control + Command + 1/2/3/4` | 把 Quick Action 切换到文本/图片/媒体/语音模式 |
| 语音模式下 `Command + 1...5` | 生成、试听音色、播放生成语音、导出 WAV、导出 M4A |

Quick Action 模式和各模式动作快捷键都可以在“设置 -> 快捷键”中修改；语音模式另保留 `Command + Return` 作为生成/停止的固定快捷键。
...
| 安装独立 VoxCPM2 TTS runtime | `LLMTOOLS_TTS_DOWNLOAD_MODEL=0 ./scripts/install-tts-voxcpm2-runtime.sh` |
| 真实 VoxCPM2 TTS smoke | `swift run LLMToolsTTSSmoke --variant voxCPM2FourBit --output /tmp/llmtools-tts.wav` |
...
Sources/
  ...
  LLMToolsTTSSmoke/     本地 VoxCPM2 与角色分析 smoke 可执行程序
browser-extension/
