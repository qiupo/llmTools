...
llmTools 仍在持续开发中。当前 `v0.3.0` 版本中，桌面 Quick Action、本地/远程模型注册表、模型能力设置、本地 MLX 视觉语言模型 runner 路径、原生模型视觉 OCR 工作流，以及 Phase 4 媒体字幕基础能力已经可用。

Phase 4.x 已加入本地语言识别、文件字幕说话人分离和 fast MT 运行时接线。实时字幕的说话人分离仍然硬禁用：当前 MVP 只允许文件字幕 diarization，不持久化 speaker embedding，也不做跨文件说话人身份识别。
...
本地 ASR runtime 通过命令模板接入，避免把音频发送到设备外。可以在 Settings -> Media -> Local ASR runtime 配置命令模板，命令模板支持 `{model}`、`{audio}`、`{language}`、`{mode}` 和 `{isFinal}`，也可以用启动时环境变量作为 fallback：
...
隐私默认保持收紧：默认不把原始音频、完整转写、字幕译文、页面标题、完整 URL 或完整媒体路径写入诊断或历史。ASR 处理后的临时归一化音频会删除。
...
| 安装 Phase 4 本地 MLX ASR runtime | `./scripts/install-phase4-mlx-asr-runtime.sh` |
...
| 安装 Phase 4 VibeVoice-ASR runtime | `./scripts/install-phase4-vibevoice-asr-runtime.sh` |
